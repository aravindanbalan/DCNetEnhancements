...
void DCNET(Ptr<Socket> socket, bool done_dcnet);
...
{
	std::string replyMessage = "Hi this is aravind. This is anonymous receiver code and its working......";
	int replyNode = 1;
...
static void SendMessage (Ptr<Socket> socket, std::string message, int src, int dest)
{
    Ptr<Packet> sendPacket =
        Create<Packet> ((uint8_t*)message.c_str(),message.size());

//std::cout<<"*****************Message : "<<message<<" "<<message.size()<<"\n";

	std::cout<< "src : "<<src<<" Dest : "<<dest<<"\n";
    MyTag sendTag;
    sendTag.SetSourceAddress(src);
 sendTag.SetDestAddress(dest);

std::cout<< "**** "<< int(sendTag.GetSourceAddress());
 std::cout<< "**** "<< int(sendTag.GetDestAddress());

    sendPacket->AddPacketTag(sendTag);
//sendTag.Print(std::cout<<"\n");
...
int recNodeIndex = int(recTag.GetDestAddress());
    ...
    std::string value = dec.c_str();
//std::cout<<"value random bit received : "<<dec<<"\n";
...
std::string PseudoRandomGenerator(unsigned int SEEDSIZE)
{
   ...
   // Scratch Area
    const unsigned int BLOCKSIZE = SEEDSIZE * 2;
...
std::cout << "The seed size is:"<<SEEDSIZE<<"\n";

    std::ostringstream ss111;
	
    for( unsigned int i = 0; i < BLOCKSIZE; i++ )
    {
        ss111 << std::setbase(16) << std::setfill('0');
        ss111 << static_cast<unsigned int>( pcbScratch[ i ] );
    }
    std::vector<char> binary= convert_to_binary(ss111.str().c_str());
    std::string binaryresult(binary.begin(),binary.end());
	
	binaryresult = binaryresult.substr(0,MessageLength);
	//std::cout << "Ascii to binary : " << binaryresult <<"\n";
	return binaryresult;
}
...
publicKeyCounter = (numNodes * numNodes) - numNodes;
    ...
    for (int index1 = 0; index1 < (int)numNodes; index1++)
    {
		...
		std::string prgString = PseudoRandomGenerator(MessageLength);
             //   std::cout<<"Random bit : "<<randomBit<<" "<<index1<<" "<<index2<<"\n";
               ...
               // std::ostringstream ss;
                ...
                messageLen = (int)strlen(prgString.c_str())+1;
		...
		sourceNodeSocket->Connect (remoteSocket);
                std::cout<<"Source : "<<i.GetAddress (index1, 0) << " Dest : " <<i.GetAddress (index2, 0)<<"\n";
		std::cout<<"Sending random bit  from " << index1 <<" to  "<<index2<<"\n";
                Simulator::ScheduleNow(&SendMessage, sourceNodeSocket,enc,index1, index2);
            }
        }
    }
...
std::cout<<"1111src node index : "<< srcNodeIndex << " 1111received node index : "<< recNodeIndex<<"\n";
    DH dh;
...
int main (int argc, char *argv[])
{
    ...
    Config::SetDefault ("ns3::WifiRemoteStationManager::FragmentationThreshold", StringValue ("7000"));
    // turn off RTS/CTS for frames below 2200 bytes
    Config::SetDefault ("ns3::WifiRemoteStationManager::RtsCtsThreshold", StringValue ("7000"));
    ...
    wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
				 ...
				 "FragmentationThreshold", UintegerValue (7000));
